...
from teamportal.database import Base, User, Ideas, Comments, Likes, WatchList, \
...
@app.route('/autcomplete', methods=['GET'])
def autcomplete():
    search = request.args.get('search')

    if search:
        try:
            results = dbsession.query(Ideas).filter(Ideas.title.like('%'+search+'%')).all()
...
def search():
    if request.method == 'POST':
            ...
            try:
                results = dbsession.query(Ideas).filter(Ideas.title.like('%'+lookup+'%'))
